...
- (void)createButtons
{
    ...
    [self.button1 setTitle:self.buttonTitles1[0] forState:UIControlStateNormal];
    ...
    [self.allButtons1 addObject:self.button1];
    [self.button1 addSubview:self.view1];
    [self.button1 addTarget:self action:@selector(showLeftData:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:self.button1];
    
    self.button2 = [[UIButton alloc]init];
    self.button2.tag = 1;
    self.button2.frame = CGRectMake((((WIDTH - 2*80)/3) + 80) * 1 + ((WIDTH - 2*80)/3), self.frame.size.height - 30, 80, 30);
    [self.button2 setTitle:self.buttonTitles1[1] forState:UIControlStateNormal];
    ...
    [self.allButtons1 addObject:self.button2];
    [self.button2 addSubview:self.view2];
    [self.button2 addTarget:self action:@selector(showLeftData:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)changeCount:(TRButton*)sender
{
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:@"buttonChange" object:self userInfo:@{@"buttonTag":[NSString stringWithFormat:@"%ld",(long)sender.tag],@"buttonArray":self.buttonTitles}];
}
